...
void * matrizPorVector(Matriz m, ObjetoCartesiano * v){
	float a1 = m.get(1,1), a2 = m.get(2,1), a3 = m.get(3,1), a4 = m.get(4,1),
	...
	if ( v->esVector() ){
		v->set_values(vR1, vR2, vR3);
	}
	else{
		float d1 = m.get(1,4), d2 = m.get(2,4), d3 = m.get(3,4), d4 = m.get(4,4);
		float vR4 = v1 * d1 + v2 * d2 + v3 * d3 + v4 * d4;
		v->set_values(vR1 / vR4, vR2 / vR4, vR3 / vR4);
	}
}